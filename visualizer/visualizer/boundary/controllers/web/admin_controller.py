...
from visualizer.lib import admin as admin_lib
...
async def admin_dashboard(request: Request, _: str = Depends(verify_admin)):
    tenants = admin_lib.list_tenants()
...
async def create_tenant(
    ...
    admin_lib.create_tenant(name, db_name)
...
async def tenant_detail(
    ...
    tenant = admin_lib.get_tenant(tenant_id)
    if not tenant:
        raise HTTPException(status_code=404, detail="Tenant not found")

    users = admin_lib.list_users_for_tenant(tenant_id)
    return templates().TemplateResponse(
...
async def create_user(
    ...
    existing = admin_lib.get_user_by_email(email)
    if existing:
        return templates().TemplateResponse(
            "admin/user_form.html",
    ...
    admin_lib.create_user(email, tenant)
...
async def delete_tenant(
    ...
    success = admin_lib.delete_tenant(tenant_id)
    if not success:
        raise HTTPException(status_code=404, detail="Tenant not found")
    return RedirectResponse(url="/admin", status_code=303)
