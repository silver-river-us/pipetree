...
def get_run_telemetry(run_id: str, db_path: Path) -> dict[str, Any]:
    """Get telemetry data for a specific run.

    Returns dict with run and steps data.
    """
    run_data = None
...
def compare_runs(
    run1_id: str, db1_path: Path, run2_id: str, db2_path: Path
) -> dict[str, Any]:
    ...
    run2_data = _get_run_data(run2_id, db2_path)
